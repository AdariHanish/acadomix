...
import { Star, Check, X, Trash2, Eye, Calendar, GraduationCap } from 'lucide-react';
...
export default function AdminReviews() {
  ...
  const loadReviews = () => ReviewsDB.getAll().then(setReviews);
  ...
  const filteredReviews = reviews.filter(r => filter === 'pending' ? !r.is_approved : filter === 'approved' ? r.is_approved : true);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        )) : <p className="text-center py-12 text-white/15 text-[13px]">No {filter !== 'all' ? filter : ''} reviews</p>}
      </div>
